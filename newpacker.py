...
class Bin:
    def __init__(self, name, width, height, depth, total_height, max_weight):
        self.name = name
        self.width = width
        self.height = height
        self.depth = depth
        ...
        self.unfitted_items = []
        self.number_of_decimals = DEFAULT_NUMBER_OF_DECIMALS
        self.heightFromGround = total_height
...
class Packer:
    ...
    def pack(
        ...
        for bin in self.bins:
            for item in self.items:
                if item.weight > 12:
                    if bin.heightFromGround > 1361:
                        continue
                if item.weight > 10:
                    bin.height = bin.height - 70
                    bin.width = bin.width - 140
